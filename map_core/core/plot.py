...
def create_map_legend(
    ...
    if figure_size is None:
        figure_size = [10, 10]
    return setup_figure_and_axes(
        figure_size, figure_dpi, background_color, margin, transparent
    )
...
def _add_legend_item(
    ...
    """
    if is_line:
        item = mpatches.Rectangle(
            (0, 0), 1, 0.1, facecolor=color, edgecolor="none", label=label
        )
    else:
        item = mpatches.Patch(
            facecolor=color, edgecolor=edgecolor, linewidth=linewidth, label=label
        )
    handles.append(item)
    labels.append(label)
...
def generate_layer_legend(
    ...
    if not has_data(data):
        ax.text(
            0.5,
            0.5,
            f"No data available for {layer_name}",
            ha="center",
            va="center",
            fontsize=12,
        )
    ...
    try:
        if layer_name == "water":
            ...
            if hasattr(data, "geometry"):
                ...
                if "natural" in data.columns:
                    for val in data["natural"].dropna().unique():
                        unique_features[f"Natural: {val}"] = layer_style.get(
                            "facecolor", "#a6cee3"
                        )
                if "waterway" in data.columns:
                    for val in data["waterway"].dropna().unique():
                        unique_features[f"Waterway: {val}"] = layer_style.get(
                            "edgecolor", "#a6cee3"
                        )
                ...
                if not unique_features:
                    unique_features = {
                        "Water features": layer_style.get("facecolor", "#a6cee3")
                    }
                ...
                for label, color in unique_features.items():
                    _add_legend_item(
                        ...
                        legend_labels,
                        color,
                        layer_style.get("edgecolor", color),
                        layer_style.get("linewidth", 0.5),
                        label,
                    )
                    ...
                    if "highway" in edges.columns:
                        ...
                        for hw_type in sorted(highway_types):
                            _add_legend_item(
                                legend_handles,
                                legend_labels,
                                layer_style.get("edgecolor", "#000000"),
                                "none",
                                0.5,
                                hw_type.capitalize(),
                                is_line=True,
            ...
            if auto_mode == "manual":
                color = layer_style.get("manual_color_settings", {}).get(
                    "facecolor", "#000000"
                )
                _add_legend_item(
                    legend_handles,
                ...
                palette_key = (
                    "auto_size_palette"
                    if auto_mode == "auto_size"
                    else "auto_distance_palette"
                )
                palette_name = layer_style.get(palette_key, "")
                palettes = load_palettes()
                colors = (
                    palettes.get(palette_name, []) if isinstance(palettes, dict) else []
                )
                ...
                for tag in ["leisure", "landuse", "natural"]:
                    if tag in data.columns:
                        for val in data[tag].dropna().unique():
                            unique_features[f"{tag.capitalize()}: {val}"] = (
                                layer_style.get("facecolor", "#b2df8a")
                            )

                if not unique_features:
                    unique_features = {
                        "Green spaces": layer_style.get("facecolor", "#b2df8a")
                    }
        ...
        if legend_handles:
            ...
            ax.text(
                0.5,
                0.5,
                f"No legend items for {layer_name}",
                ha="center",
                va="center",
                fontsize=12,
            )
...
def plot_map_layer(
    ...
    """
    from map_core.core.util import (has_data,  # local import to avoid cycles
                                    log_progress)
    ...
    if layer_name == "streets":
        ...
        if hasattr(data, "geometry") and not hasattr(data, "graph"):
            try:
                ...
                log_progress(
                    f"Warning: failed to plot streets from GeoDataFrame directly: {e}"
                )
        ...
        try:
            ...
            log_progress(
                f"Warning: failed to plot streets via edges GDF ({e}); falling back to ox.plot_graph."
            )
            try:
                ox.plot_graph(
                    data,
        ...
        if layer_name == "water" and has_data(data):
            _plot_water_features(
                ax, data, facecolor, edgecolor, linewidth, alpha, hatch, zorder
            )
        else:
...
def _plot_water_features(
    ...
    if not all(gdf.geometry.is_valid):
        log_progress(
            f"Found {sum(~gdf.geometry.is_valid)} invalid geometries in water layer, fixing..."
        )
    ...
    if not polys.empty:
        ...
        river_polys = (
            polys[polys.get("water", "") == "river"]
            if "water" in polys.columns
            else pd.DataFrame()
        )
        other_polys = (
            polys[polys.get("water", "") != "river"]
    ...
    if not lines.empty:
        ...
        rivers = (
            lines[lines.get("waterway", "") == "river"]
            if "waterway" in lines.columns
            else pd.DataFrame()
        )
        streams = (
            lines[lines.get("waterway", "") == "stream"]
            if "waterway" in lines.columns
            else pd.DataFrame()
        )
        canals = (
            lines[lines.get("waterway", "") == "canal"]
            if "waterway" in lines.columns
            else pd.DataFrame()
        )
        other_waterways = lines[
            ~lines.index.isin(rivers.index)
        ...
        if not rivers.empty:
            for idx, row in rivers.iterrows():
                ...
                single_gdf = gpd.GeoDataFrame(
                    [row], geometry="geometry", crs=rivers.crs
                )
                single_gdf.plot(
                    ax=ax,
                    color=river_blue,
        ...
        if not streams.empty:
            for idx, row in streams.iterrows():
                ...
                single_gdf = gpd.GeoDataFrame(
                    [row], geometry="geometry", crs=streams.crs
                )
                single_gdf.plot(
                    ax=ax,
                    color=stream_blue,
        ...
        if not canals.empty:
            for idx, row in canals.iterrows():
                ...
                lw = max(linewidth, width_m / meters_per_point)
                single_gdf = gpd.GeoDataFrame(
                    [row], geometry="geometry", crs=canals.crs
                )
                single_gdf.plot(
                    ax=ax,
                    color=river_blue,
    ...
    try:
        ...
        if legend_handles:
            ...
            ax.text(
                0.5,
                0.5,
                f"No feature types found for {layer_name}",
                ha="center",
                va="center",
                fontsize=12,
            )
...
def generate_debug_map(
    ...
    fig, ax = setup_figure_and_axes(
        figure_size, figure_dpi, background_color, margin, transparent=False
    )
...
def _plot_water_debug(ax, data, legend_handles, legend_labels):
    ...
    for feature_key, color in feature_types.items():
        ...
        if len(subset) > 0:
            ...
            if is_polygon:
                subset.plot(
                    ax=ax, facecolor=color, edgecolor="black", linewidth=0.3, alpha=0.7
                )
            else:
...
def _plot_streets_debug(ax, data, legend_handles, legend_labels):
    ...
    try:
        ...
        if isinstance(data, gpd.GeoDataFrame):
            ...
            log_progress(
                f"Warning: Expected NetworkX graph or GeoDataFrame for streets debug, got {type(data)}"
            )
            return
        ...
        if "highway" in edges.columns:
            log_progress(
                f"[debug] Unique highway values: {edges['highway'].dropna().unique().tolist()}"
            )

        if "highway" not in edges.columns or edges["highway"].dropna().empty:
            log_progress(
                "[debug] 'highway' column not found or empty in street edges. Plotting all streets in gray."
            )
        ...
        for hw_type, color in sorted(highway_types.items()):
            ...
            mask = edges["highway"].apply(
                lambda x: hw_type in x if isinstance(x, list) else x == hw_type
            )
            subset = edges[mask]
...
def _plot_buildings_debug(ax, data, legend_handles, legend_labels):
    ...
    if not building_types:
        data.plot(
            ax=ax, facecolor="#ff7f0e", edgecolor="black", linewidth=0.3, alpha=0.7
        )
        legend_handles.append(
            mpatches.Rectangle((0, 0), 1, 1, facecolor="#ff7f0e", edgecolor="black")
        )
    ...
    for btype, color in sorted(building_types.items()):
        ...
        if len(subset) > 0:
            ...
            legend_handles.append(
                mpatches.Rectangle((0, 0), 1, 1, facecolor=color, edgecolor="black")
            )
...
def _plot_green_debug(ax, data, legend_handles, legend_labels):
    ...
    if not feature_types:
        data.plot(
            ax=ax, facecolor="#90ee90", edgecolor="black", linewidth=0.3, alpha=0.7
        )
        legend_handles.append(
            mpatches.Rectangle((0, 0), 1, 1, facecolor="#90ee90", edgecolor="black")
        )
...
def _collect_streets_legend(data, legend_handles, legend_labels):
    ...
    try:
        ...
        if isinstance(data, gpd.GeoDataFrame):
            ...
            log_progress(
                f"Warning: Expected NetworkX graph or GeoDataFrame for streets legend, got {type(data)}"
            )
            return
        ...
        if "highway" not in edges.columns or edges["highway"].dropna().empty:
            log_progress(
                "[debug] 'highway' column not found or empty in street edges. Adding generic legend entry."
            )
            legend_handles.append(mpatches.Patch(color="gray"))
            legend_labels.append("Streets (no type info)")
            return
