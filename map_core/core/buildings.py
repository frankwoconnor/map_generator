    ...
    try:
        ...
        if original_crs and original_crs.is_geographic:
            try:
                ...
                log_progress(
                    f"Warning: Could not reproject buildings to UTM for metric '{metric}': {e}. Using original CRS."
                )
                data_proj = data
        else:
            data_proj = data
        ...
        log_progress(
            f"Warning: failed to compute buildings metric '{metric}': {e}. Using zeros."
        )
    return pd.Series([0] * len(data), index=getattr(data, "index", None))
